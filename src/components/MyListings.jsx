...
const MyListings = () => {
  ...
  // Allow parent components to get the tool count for dashboard stats
  const updateStatsIfNeeded = (toolsData) => {
    if (window.location.pathname === '/seller/dashboard' && window.updateDashboardStats) {
      const activeListings = toolsData.filter(tool => !tool.is_sold).length;
      const soldItems = toolsData.filter(tool => tool.is_sold).length;
      window.updateDashboardStats({ activeListings, soldItems });
    }
  };
  ...
  const fetchTools = async () => {
    try {
      ...
      // Update dashboard stats if this component is on the dashboard
      updateStatsIfNeeded(toolsData);
    } catch (error) {
  ...
  // Determine if this component is being rendered on the dashboard page
  const isDashboardPage = window.location.pathname === '/seller/dashboard';

  // If it's being rendered standalone, use the full layout; if on dashboard, use a more compact layout
  return (
    <>
      {!isDashboardPage && (
        <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      {isDashboardPage && (
        <h2 className="text-2xl font-medium mb-6">My Listings</h2>
      )}

      <div className={isDashboardPage ? "" : "max-w-7xl mx-auto px-4"}>
        {error && (
              ...
              return (
                <div key={tool.id} className="border rounded-lg overflow-hidden bg-white shadow-sm">
                  ...
                  <div className="border-t p-4 flex justify-between items-center">
                    <Link 
                      to={`/tool/${tool.id}`}
                      className="text-stone-600 hover:text-forest-700 flex items-center"
                    >
